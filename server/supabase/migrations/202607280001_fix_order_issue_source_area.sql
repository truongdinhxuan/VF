-- Issue stock only from a storage location owned by the order's source area.
-- Replacing the function preserves the existing API/RPC contract while making
-- the database transaction the authoritative guard against forged location IDs.
...
begin
  ...
  loop
    ...
    loop
      ...
      select location.*
        into v_location
      from public.storage_locations as location
      where location.id = (v_issue_payload ->> 'storage_location_id')::uuid
        and location.area_id = v_order.from_area_id
        and location.is_active = true
        -- Supports both the current schema and a clean migration chain where
        -- storage_locations may not yet expose is_deleted as a physical column.
        and lower(coalesce(to_jsonb(location) ->> 'is_deleted', 'false')) = 'false';

      if not found then
        raise exception
          'Storage location is inactive, deleted, or does not belong to the order source area';
      end if;
      ...
      where supply_id = v_order_item.supply_id
        and area_id = v_order.from_area_id
        and storage_location_id = v_location.id
      for update;

      if not found then
        raise exception 'Stock balance not found for supply in the order source area';
      end if;
