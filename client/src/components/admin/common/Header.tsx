    ...
    <header className="z-20 mx-4 mt-4 flex min-h-14 shrink-0 items-center justify-between rounded-2xl border border-slate-100 bg-white px-4 shadow-sm sm:mx-6 sm:mt-6 sm:min-h-16 sm:px-6 dark:border-slate-800 dark:bg-slate-800/80 dark:backdrop-blur">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="mobile-menu-toggle block md:hidden rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-slate-100 dark:text-stone-400 dark:hover:bg-slate-700"
        >
        ...
        <button
          ...
          className="hidden md:block rounded-lg p-1.5 text-slate-500 transition-colors hover:bg-slate-100 dark:text-stone-400 dark:hover:bg-slate-700"
        >
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative hidden lg:block">
          ...
          <input
            ...
            className="w-64 rounded-xl border-none bg-slate-100/50 py-2 pl-10 pr-4 text-sm font-medium text-slate-700 transition-all placeholder:text-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-slate-700 dark:text-stone-200"
          />
          {searchQuery && (
        ...
        <div className="relative" ref={notificationRef}>
          <button
            ...
            className="relative cursor-pointer rounded-lg p-1.5 text-slate-500 transition-colors hover:text-slate-800 dark:text-stone-400 dark:hover:text-stone-200 hover:bg-slate-100 dark:hover:bg-slate-700"
          >
            <span className="absolute right-1 top-1 h-2 w-2 rounded-full border border-white bg-red-500 dark:border-transparent"></span>
            <i className="hgi-stroke hgi-notification-01 text-2xl"></i>
          </button>

          {isNotificationsOpen && (
            <div className="absolute right-0 z-50 mt-3 w-80 rounded-2xl border border-slate-100 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-800">
              <div className="flex items-center justify-between border-b border-slate-100 p-4 dark:border-slate-700">
                <h3 className="font-bold text-slate-800 dark:text-stone-200">Notifications</h3>
                <button onClick={() => console.log("Marked read")} className="text-xs font-semibold text-blue-600 hover:text-blue-700 dark:text-blue-400">
                  Mark all as read
                </button>
              </div>
              <div className="max-h-80 overflow-y-auto">
                <div className="flex items-start gap-4 border-b border-slate-50 p-4 transition-colors hover:bg-slate-50 dark:border-slate-700/50 dark:hover:bg-slate-700/50">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-800 dark:text-stone-200">New customer signed up</p>
                    <p className="text-xs text-slate-500 dark:text-stone-400">Sarah Connor joined pro plan.</p>
