...
const RoleDashboardPage = ({ workspaceRole }: { workspaceRole: RoleCode }) => {
  ...
  const workspace = ROLE_WORKSPACES[workspaceRole];
  ...
  return (
    <section className="space-y-6">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <p className="text-xs font-bold uppercase tracking-widest text-blue-600">{workspace.dashboardLabel}</p>
        <h1 className="mt-2 text-2xl font-bold text-slate-900">Xin chào, {displayName}</h1>
        <p className="mt-2 text-sm text-slate-500">
          {workspace.dashboardDescription}
        </p>
      </div>
